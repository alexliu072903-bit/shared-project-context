...
当前 installer 先支持单人本地试用。设计师和工程师分别安装后，并不会自动连接到同一个 shared project。

进入真实多人试用前，还需要创建一个独立的 private shared project repository，并把每个人发布出的项目更新汇入该 repository。这个多人连接和初始化流程尚未包含在当前版本中。
